...
const directionalLight = new THREE.DirectionalLight("whtie", 0.6);
...
const directionalPane = pane.addFolder({
  title: "Direction Light",
  extended: true,
});
directionalPane.addInput(directionalLight, "color", {
  color: { type: "float" },
});
directionalPane.addInput(directionalLight, "intensity", {
  min: 0,
  max: 1,
  step: 0.01,
});
directionalPane.addInput(directionalLight, "position", {
  x: { min: 0, max: 10, step: 1 },
  y: { min: 0, max: 10, step: 1 },
  z: { min: 0, max: 10, step: 1 },
});
